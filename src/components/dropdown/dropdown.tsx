            ...
            <AnimatePresence>
                {(display ?? isOpen) && (
                    <motion.div
                        ...
                        exit={{ opacity: 0, y : -10 }}
                        className={`absolute overflow-y-auto right-0 mt-2 min-w-[150px] max-h-[300px] w-full bg-bg-primary border border-border-primary rounded-xl shadow-lg z-50 py-1 pt-[6px] ${className}`}
                    >
                        {
                            menuItems &&
                            <div className="flex flex-col">
                                {menuItems.map((item) =>
                                    item.type === "divider" ? (
                                        <div key={item.key} className="border-t border-border-secondary my-1" />
                                    ) : (
                                        <div
                                            key={item.key}
                                        ...
                                        >
                                            {item.type === "link" ? (
                                                <Link href={item.href ?? "#"}>
                                                    <div
                                                        className={`${
                                                            !item.disabled ? "hover:bg-bg-tetiary px-2 py-[3px] rounded-md flex gap-[6px] items-center" : "flex"
                                                        }`}
                                                    >
